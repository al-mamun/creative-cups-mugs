...
function getCategoryFromTags(tags) {
  ...
  const firstValidTag = tags.find(
    (tag) => !ignoreTags.includes(tag.toLowerCase())
  );
    ...
    categories.forEach((category) => {
      ...
      tabsHTML += `
        <label class="style-tab">
          <input type="radio" name="style-type" value="${category}" ${
        isFirst ? "checked" : ""
      } />
...
function renderStyleProducts(products, container) {
  ...
  products.forEach((product) => {
    ...
    productsHTML += `
      <div class="style-item" data-category="${category}" data-price="${price}" data-id="${
      product.id
    }">
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function initializeCupOptions() {
    ...
    try {
      ...
      if (
        window.customDesignerData?.useCollections &&
        window.customDesignerData?.cupCollection
      ) {
        ...
        products = await fetchProductsFromCollection(
          window.customDesignerData.cupCollection
        );
        ...
        if (productHandles.length === 0) {
          cupGrid.innerHTML =
            '<p class="error-message">No cup products added. Please add "Cup Product" blocks or enable Collection Mode in theme customizer.</p>';
          return;
        }
      ...
      const validProducts = products.filter((p) => {
        ...
        if (defaultVariant.inventory_management) {
          ...
          const allowsBackorder =
            defaultVariant.inventory_policy === "continue";
          ...
          if (!hasStock) {
            console.log(
              ...
              qty,
              "Policy:",
              defaultVariant.inventory_policy
            );
            return false;
          }
        }
      ...
      if (validProducts.length === 0) {
        cupGrid.innerHTML =
          '<p class="error-message">No cup products available in stock. Please check back later.<br><small>Check browser console (F12) for debug info.</small></p>';
        return;
      }
      ...
      cupGrid.innerHTML =
        '<p class="error-message">Error loading cups. Please refresh the page.</p>';
    }
  }
  ...
  async function initializeWrapGallery() {
    ...
    try {
      ...
      if (
        window.customDesignerData?.useCollections &&
        window.customDesignerData?.wrapCollection
      ) {
        // COLLECTION MODE - Unlimited products!
        console.log("🚀 WRAP COLLECTION MODE ENABLED");
        console.log(
          "Wrap Collection Handle:",
          window.customDesignerData.wrapCollection
        );

        const collectionProducts = await fetchProductsFromCollection(
          window.customDesignerData.wrapCollection
        );
        ...
        if (wrapProductsData.length === 0) {
          wrapGallery.innerHTML =
            '<p class="error-message">No wrap products added. Please add "Wrap Product" blocks or enable Collection Mode in theme customizer.</p>';
          return;
        }
        ...
        const fetchedProducts = await Promise.all(
          wrapProductsData.map(async (productData) => {
            try {
              const response = await fetch(
                `/products/${productData.handle.trim()}.js`
              );
              ...
              console.error(
                `Error fetching wrap product ${productData.handle}:`,
                error
              );
              return null;
            }
      ...
      if (validWrapProducts.length === 0) {
        wrapGallery.innerHTML =
          '<p class="error-message">No wrap products available in stock. Please check back later.</p>';
        return;
      }
      ...
      const updatedCategoryTabs = document.querySelectorAll(
        ".wrap-categories .category-tab"
      );
      ...
      wrapGallery.innerHTML =
        '<p class="error-message">Error loading wrap products. Please refresh the page.</p>';
    }
  }
  ...
  function renderWrapProducts(products) {
    ...
    products.forEach((product) => {
      ...
      productHTML += `
        <div class="wrap-item" data-category="${category}" data-product-handle="${
        product.handle
      }">
  ...
  function toggleWrapProductSelection(e) {
    ...
    const index = customization.selectedWraps.findIndex(
      (w) => w.id === wrapData.id
    );
    ...
    if (index > -1) {
      ...
      if (
        wrapData.inventoryManagement !== "null" &&
  ...
  function updateSelectedWrapsDisplay() {
    ...
    customization.selectedWraps.forEach((wrap) => {
      ...
      item.innerHTML = `
        ${
          wrap.image
            ? `<img src="${wrap.image}" alt="${wrap.title}" class="selected-wrap-item__image" />`
            : ""
        }
        <span class="selected-wrap-item__name">${wrap.title}</span>
        <span class="selected-wrap-item__price">+$${wrap.price.toFixed(
          2
        )}</span>
        <button class="selected-wrap-item__remove" data-wrap-id="${
          wrap.id
        }" aria-label="Remove wrap">
      ...
      removeBtn.addEventListener("click", () => {
        ...
        if (index > -1) {
          ...
          // Update UI
          const wrapItem = document
            .querySelector(
              `.wrap-item .wrap-item__select[data-wrap-id="${wrapId}"]`
            )
            ?.closest(".wrap-item");
          if (wrapItem) {
            wrapItem.classList.remove("selected");
  ...
  async function initializeStyleGallery() {
    ...
    if (
      ...
      try {
        ...
        styleGallery.innerHTML =
          '<div class="loading-spinner">Loading styles...</div>';
        ...
        const validProducts = productsWithCategories.filter((p) => {
          ...
          if (
            !defaultVariant.inventory_management &&
            !defaultVariant.available
          ) {
        ...
        if (validProducts.length === 0) {
          styleGallery.innerHTML =
            '<p class="error-message">No style products available. Please check back later.</p>';
          return;
        }
        ...
        styleGallery.innerHTML =
          '<p class="error-message">Error loading styles. Please refresh the page.</p>';
      }
    ...
    // Style selection - use event delegation to avoid duplicate listeners
    initializeStyleSelection();
  ...
  function updateSelectedStylesDisplay() {
    ...
    customization.selectedStyles.forEach((style) => {
      ...
      item.innerHTML = `
        ${
          style.image
            ? `<img src="${style.image}" alt="${style.title}" class="selected-style-item__image" />`
            : ""
        }
        <span class="selected-style-item__name">${style.title}</span>
        <span class="selected-style-item__price">+$${style.price.toFixed(
          2
        )}</span>
        <button class="selected-style-item__remove" data-style-id="${
          style.id
      ...
      removeBtn.addEventListener("click", () => {
        ...
        if (index > -1) {
          ...
          // Update UI
          const styleItem = document
            .querySelector(
              `.style-item .style-item__select[data-style-id="${styleId}"]`
            )
            ?.closest(".style-item");
          if (styleItem) {
            styleItem.classList.remove("selected");
  ...
  function updatePreviewSummary() {
    ...
    if (wrapsSection) {
      if (customization.selectedWraps.length > 0) {
        ...
        if (wrapsList) {
          wrapsList.innerHTML = customization.selectedWraps
            .map(
              (wrap) =>
    ...
    if (stylesSection) {
      if (customization.selectedStyles.length > 0) {
        ...
        if (stylesList) {
          stylesList.innerHTML = customization.selectedStyles
            .map(
              (style) =>
                `<div class="preview-summary__item">
  ...
  if (checkoutBtn) {
    checkoutBtn.addEventListener("click", () => {
      ...
      const properties = {
        ...
        "_Watch on TikTok Live":
          customization.tiktokLive === "yes" ? "Yes" : "No",
        "_Customization Details": `Cup: $${customization.cupPrice.toFixed(
          2
        )} | Wraps: $${wrapTotal.toFixed(2)} | Styles: $${styleTotal.toFixed(
          2
      ...
      if (customization.selectedWraps.length > 0) {
        properties["_Wrap Designs"] = customization.selectedWraps
          .map((w) => w.title)
          .join(", ");
      }
      ...
      if (customization.selectedStyles.length > 0) {
        properties["_Style & Colors"] = customization.selectedStyles
          .map((s) => s.title)
          .join(", ");
      }
      ...
      if (totalPrice > 100) {
        showNotification(
          "⚠️ Your customization exceeds $100. Please contact us for a custom quote!",
          "error"
        );
      ...
      fetch("/products/custom-design-product.js")
        .then((response) => {
          if (!response.ok) {
            throw new Error(
              "Custom Design Product not found. Please create it using the CSV import or follow QUICK_SETUP_5_MINUTES.md instructions."
            );
          }
        ...
        .then((product) => {
          ...
          if (!selectedVariant) {
            throw new Error(
              "No suitable price variant found. Please ensure Custom Design Product has 100 price variants."
            );
          }
          ...
          const customProperties = {
            ...
            "Wrap Designs": customization.selectedWraps
              .map((w) => w.title)
              .join(", "),
            "Wrap Cost": `$${wrapTotal.toFixed(2)}`,
            "Style & Colors": customization.selectedStyles
              .map((s) => s.title)
              .join(", "),
